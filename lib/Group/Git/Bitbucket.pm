...
sub _repos {
    ...
    my $user = $conf->{user} ? $conf->{user} : httpenc prompt( -prompt => 'bitbucket username : ' );
    my $pass = $conf->{pass} ? $conf->{pass} : httpenc prompt( -prompt => 'bitbucket password : ', -echo => '*' );
...
Group::Git::Bitbucket - <One-line description of module's purpose>

=head1 VERSION

This documentation refers to Group::Git::Bitbucket version 0.1.
